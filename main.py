...
def search_moves(command, state):
	...
	for move in yuried_moves:
		print("info currmove {0} score cp {1} nodes {2} depth 1 seldepth 2 pv {0} hashfull 0".format(move["Name"], int(state.current_eval_func(move)), len(yuried_moves) * 2))
		print("info currmove {0} score cp {1} nodes {2} depth 1 seldepth 2 pv {0} hashfull 0".format(move["Name"], int(state.backup_eval_func(move)), len(yuried_moves) * 2))
